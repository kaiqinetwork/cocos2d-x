...
int lua_cocos2dx_spine_SkeletonRenderer_composeSkin(lua_State* tolua_S)
{
    int argc = 0;
    spine::SkeletonRenderer* cobj = nullptr;
    bool ok = true;
    ...
    if (!cobj)
    {
        tolua_error(tolua_S, "invalid 'cobj' in function 'lua_cocos2dx_spine_SkeletonRenderer_composeSkin'", nullptr);
        return 0;
    }
#endif
    argc = lua_gettop(tolua_S) - 1;
    do {
        if (argc == 2) {
            ...
            int arg1;
            ok &= luaval_to_std_vector_string(tolua_S, 2, &arg0, "sp.SkeletonRenderer:composeSkin");
            ok &= luaval_to_int32(tolua_S, 3, &arg1, "sp.SkeletonRenderer:composeSkin");

            if (!ok) { break; }
            const char** names = new const char* [arg0.size() + 1];
            for (size_t i = 0; i < arg0.size(); i++) {
                names[i] = arg0[i].c_str();
            }
            bool ret = cobj->composeSkin(names, arg1);
            tolua_pushboolean(tolua_S, (bool)ret);
            return 1;
        }
    } while (0);
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "sp.SkeletonRenderer:composeSkin", argc, 1);
    return 0;

#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S, "#ferror in function 'lua_cocos2dx_spine_SkeletonRenderer_composeSkin'.", &tolua_err);
#endif
...
int lua_register_cocos2dx_spine_SkeletonRenderer(lua_State* tolua_S)
{
    ...
    tolua_beginmodule(tolua_S,"SkeletonRenderer");
        ...
        tolua_function(tolua_S, "setSkin", lua_cocos2dx_spine_SkeletonRenderer_setSkin);//皮肤//
        tolua_function(tolua_S, "composeSkin", lua_cocos2dx_spine_SkeletonRenderer_composeSkin);//合成换皮 局部组合//
